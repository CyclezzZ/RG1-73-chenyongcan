...
public class SignAction extends BaseAction {
	...
	@Action(value = "toSelfPassword", results = { @Result(name = "modifySelf", location = "/ahtml/modifySelfPassword.jsp") })
	public String toSelfPassword() {
		return "modifySelf";
	}

	@Action(value = "modifySelfPassword")
	public String modifySelfPassword() {
		try {
			...
			sessionUser = service.unique("from User where userId=?", sessionUser.getUserId());
			MD5 md = new MD5();
			String oldpwd = md.getMD5ofStr(getHttpServletRequest().getParameter("oldpwd"));
			if (!oldpwd.equals(sessionUser.getUserPassword())) {
				MessageUtil.addMessage(getHttpServletRequest(), "修改失败.原密码错误");
				return ERROR;
			} else {
				log.info(bean.getUserPassword());
